...
theme_song = pyglet.media.load('./resources/dun_nun_nun.wav') 

music = pyglet.media.Player()
music.queue(theme_song)

# Set up the two top labels
score_label = pyglet.text.Label(text="RIFFS = 0", x=15, y=75, batch=main_batch, font_name = 'Arial', bold = True )
...
clock = goblin.Goblin(x=randint(0, WIDTH), y=randint(0,HEIGHT), batch=main_batch)
# Store all objects that update each frame in a list
game_objects = [hero, goblin, clock]
...
def game_over():
    ...
    is_drawing = False  
    music.pause() 
    you_won_sound = pyglet.media.load('./resources/win.wav') 
    music.queue(you_won_sound) 
    music.play()
...
def update(dt):
    ...
    if is_drawing and score < 100:
        ...
        for to_remove in [obj for obj in game_objects if obj.dead]:
            ...
            score += 10
            score_label.text = f"RIFFS = {score}"
